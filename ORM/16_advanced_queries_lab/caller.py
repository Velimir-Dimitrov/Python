...
def product_quantity_ordered() -> str:
    product_joined_orderproduct = Product.objects.annotate(sum_of_product=Sum("orderproduct__quantity")).exclude(sum_of_product=None).order_by("-sum_of_product")

    return "\n".join(f"Quantity ordered of {product_and_quantity.name}: {product_and_quantity.sum_of_product}"
                     for product_and_quantity in product_joined_orderproduct)


# # Test Code - Exercise 2
...
def ordered_products_per_customer() -> str:
    # # 5 queries
    # result = []
    # for order in Order.objects.prefetch_related("customer").order_by("id"):
    #     result.append(f"Order ID: {order.id}, Customer: {order.customer.username}")
    #     for product in order.products.select_related("category"):
