...
from main_app.choices import OStypes
# Set up Django
...
def update_operation_systems() -> None:
    Laptop.objects.update(
        operation_system=Case(
            When(brand="Asus", then=Value(OStypes.WINDOWS)),
            When(brand="Apple", then=Value(OStypes.MACOS)),
            When(brand__in=("Dell", "Acer"), then=Value(OStypes.LINUX)),
            When(brand="Lenovo", then=Value(OStypes.CHROME))
        )
    )
